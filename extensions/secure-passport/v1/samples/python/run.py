...
def mock_agent_core_handler(
    message: A2AMessage, passport: CallerContext | None
):  # CORRECTED: Signature uses A2AMessage
    ...
    if passport and passport.is_verified:
        ...
        print(
            f'  [Agent Core] Executing task with verified context: Currency={currency}, Tier={tier}'
        )
...
def create_and_run_passport_test(
    client_id: str, session_id: str | None, state: dict, signature: str | None, use_case_title: str
):
    ...
    client_passport = CallerContext(
        client_id=client_id, session_id=session_id, signature=signature, state=state
    )
    ...
    message_over_wire = SecurePassportExtension.client_middleware(
        next_handler=mock_transport_send, message=client_message, context=client_passport
    )
    ...
    SecurePassportExtension.server_middleware(
        next_handler=mock_agent_core_handler, message=message_over_wire
    )
...
def run_all_samples():
    ...
    create_and_run_passport_test(
        ...
        state={'destination': 'Bali, Indonesia', 'loyalty_tier': 'Platinum'},
    ...
    create_and_run_passport_test(
        ...
        session_id=None,
        state={'query_type': 'quarterly_revenue', 'access_scope': ['read:finance_db']},
