...
# Use the real API key if found, otherwise fallback to a demo key so reviewers can run it immediately
TRACE_API_KEY = os.getenv('TRACE_API_KEY', 'sk_live_trace_demo_key_999')
...
def mock_agent_core_handler(message: A2AMessage) -> str:
...
def simulate_incoming_request(caller_id: str, task_name: str) -> None:
    ...
    try:
        ...
        result = trace_middleware.server_middleware(
            next_handler=mock_agent_core_handler, message=incoming_message, caller_id=caller_id
        )
        print(f'\n[SUCCESS]: {result}')
