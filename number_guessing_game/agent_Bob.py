...
from utils.protocol_wrappers import cancel_task, extract_text, send_followup, send_text
...
def _negotiate_sorted_history(max_attempts: int = MAX_NEGOTIATION_ATTEMPTS) -> int:
    ...
    print(f"[Bob] Received Task {resp_task.id} with state={resp_task.status.state}")

    while resp_task.status.state == TaskState.input_required and attempts < max_attempts:
        ...
        if isinstance(maybe_hist, list) and is_sorted_history(maybe_hist):
            ...
            print("(We do sorting by random to illustrate multi-turn communication)")
        ...
        if not isinstance(resp_obj, Task):
            print("[Bob] Did not receive Task in response; aborting negotiation")
            break
        resp_task = resp_obj
        print(f"[Bob] Carol responded with state={resp_task.status.state}")

    if resp_task.status.state == TaskState.input_required and attempts >= max_attempts:
        print(f"[Bob] Reached maximum attempts ({max_attempts}). Cancelling task.")
