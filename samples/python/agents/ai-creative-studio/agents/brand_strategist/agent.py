...
IMPORTANT: Today's date is {_today_str} (Year: {_today_year}).
When conducting research, focus on current trends and data from {_today_year}, not outdated information.
Use search queries like "[topic] trends {_today_year}" to get the most recent insights.
...
- Researching current social media trends ({_today_year})
...
1. Use google_search to research the target audience's current interests and behaviors (include "{_today_year}" in searches)
2. Search for and analyze 2-3 competitor brands in the same space
3. Identify 3-5 trending topics related to the product category (current {_today_year} trends)
...
**Audience Insights:**
[Key behaviors, preferences, and pain points based on {_today_year} research]

**Competitive Analysis:**
[What 2-3 competitors are doing - their strengths and weaknesses]

**Trending Topics:**
[3-5 relevant trends to consider in {_today_year}]
...
if __name__ == '__main__':
    ...
    # Start server
    logger.info(
        '🚀 Starting Brand Strategist A2A Server on %s://%s:%s',
        PROTOCOL,
        HOST,
        PORT,
    )
