...
async def deploy_single_agent(
    ...
    # Run deployment
    returncode, _stdout, stderr = await run_command_async(cmd, cwd=agent_path)
...
async def get_service_url(
    ...
    returncode, stdout, _stderr = await run_command_async(cmd)
...
async def update_agent_a2a_config(
    ...
    env_vars_update = (
        f'PUBLIC_HOST={public_host},PUBLIC_PORT=443,PROTOCOL=https'
    )
    ...
    returncode, _stdout, stderr = await run_command_async(cmd)

    if returncode == 0:
        print(f'   ✓ A2A config updated for {service_name}')
    else:
        print(
            f'   Warning: Could not update A2A config for {service_name}: {stderr}'
        )
...
async def deploy_all_agents(project_id: str, region: str) -> dict[str, str]:
    ...
    # Build URL mapping
    agent_urls = {}
    for agent, url in zip(AGENTS, results, strict=False):
        if url:
            agent_urls[agent['name']] = url
        else:
            print(
                f'⚠️  Warning: {agent["name"]} deployment failed or URL not available'
            )

    print('\n' + '=' * 70)
    print(
        f'✓ Deployment complete! {len(agent_urls)}/{len(AGENTS)} agents deployed'
    )
    print('=' * 70 + '\n')
...
async def main_async():  # noqa: ANN201
    """Async main function."""
