...
def deploy_orchestrator(auto_deploy_specialists=False):  # noqa: ANN001, ANN201, PLR0915
    ...
    if auto_deploy_specialists:
        ...
        import env_utils  # noqa: PLC0415

        from deploy_all_specialists import deploy_all_agents  # noqa: PLC0415
        ...
        if not agent_urls:
            ...
            print(
                '   Cannot proceed with orchestrator deployment without agent URLs'
            )
            sys.exit(1)
        ...
        global COPYWRITER_URL, DESIGNER_URL, STRATEGIST_URL, CRITIC_URL, PM_URL  # noqa: PLW0603
    ...
    from creative_director.agent import (  # noqa: PLC0415
        root_app,  # App object with compaction config
    )
    ...
    try:
        ...
        print(
            f'python3 {__file__} --action test --resource_name "{resource_name}"'
        )

        return agent_engine_resource, resource_name  # noqa: TRY300
...
async def test_deployed_agent(resource_name: str) -> None:
...
def cleanup_agent_engine(resource_name: str) -> None:
...
def main() -> None:
    ...
    if args.action == 'deploy':
        try:
            _remote_app, resource_name = deploy_orchestrator(
