...
class CurrencyAgentExecutor(AgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        if TRACEABILITY_EXTENSION_URI in context.requested_extensions:
            ...
            if os.getenv("ENV", "production") != "production":
                ...
                logger.warning("Traceability Extension requested but disabled in production")
        ...
        try:
            async for event in self._runner.run_async(user_id=context_id, session_id=context_id, new_message=content): # type: ignore
                ...
                if event.is_final_response():
                    await self._process_final_response(event, updater, response_trace, task)
    ...
    def _process_trace_events(self, event, response_trace: ResponseTrace) -> None:
    ...
    async def _process_final_response(self, event, updater: TaskUpdater, response_trace: ResponseTrace | None, task) -> None:
        ...
        metadata = {TRACE_KEY: response_trace.as_dict()} if response_trace else {}
        ...
        try:
            ...
            if agent_response.status == 'completed':
                ...
                await self._update_task_status(updater, TaskState.completed, message, task, metadata=metadata, final=True)
            elif agent_response.status == 'input-required':
                await self._update_task_status(updater, TaskState.input_required, message, task, metadata=metadata, final=True)
            elif agent_response.status == 'failed':
                await self._update_task_status(updater, TaskState.failed, message, task, metadata=metadata, final=True)
        except Exception:
            # Fallback to standard failure if JSON parsing fails
            await self._update_task_status(updater, TaskState.failed, final_response_text, task, metadata=metadata, final=True)
    ...
    async def ensure_session(self, context_id: str):
