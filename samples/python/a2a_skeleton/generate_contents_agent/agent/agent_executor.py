...
from agent.agent import GenerateContentsAgent
from a2a.types import AgentCard
import logging
logger = logging.getLogger("generate_contents_agent.agent_executor")

class GenerateContentsAgentExecutor(AgentExecutor):
    def __init__(self):
        self.agent = GenerateContentsAgent()
                    ...
                    await event_queue.enqueue_event(
                        TaskArtifactUpdateEvent(
                            ...
                            artifact=new_text_artifact(
                                name='generate_contents_result',
                                description='콘텐츠 생성 결과',
                                text=text,
                            ),
                            append=False,
                            lastChunk=True,
