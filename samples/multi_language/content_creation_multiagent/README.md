...
- Create a .env file in the `content_planner_agent` directory as follows:
```bash
...
- Create a .env file in the `content_writer_agent` directory as follows:

```bash
cd ../content_writer_agent
cp .env.example .env
```
...
- Create a .env file in the `content_editor_agent_js` directory as follows:
```bash
cd ../content_editor_agent_js
cp .env.example .env
```
...
- Create a .env file in the `content_editor_agent_java` directory as follows:

```bash
cd ../content_editor_agent_java
cp .env.example .env
```
...
```bash
cd a2a-samples/samples/multi_language/content_creation_multiagent/content_planner_agent
uv run .
```

## 2. Run Content Writer Agent
```bash
cd a2a-samples/samples/multi_language/content_creation_multiagent/content_writer_agent
mvn quarkus:dev
```

## 3. Run JS Content Editor Agent
```bash
cd a2a-samples/samples/multi_language/content_creation_multiagent/content_editor_agent_js
npm install
...
```bash
cd a2a-samples/samples/multi_language/content_creation_multiagent/host_agent
uv run .
```
...
```bash
cd a2a-samples/samples/multi_language/content_creation_multiagent/content_editor_agent_java
mvn quarkus:dev
```
