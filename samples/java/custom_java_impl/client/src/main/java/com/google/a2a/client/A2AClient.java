...
public class A2AClient {
     ...
     */
    public JSONRPCResponse sendTask(final TaskSendParams params)
     ...
     * @throws A2AClientException if the request fails
     */
    public JSONRPCResponse getTask(final TaskQueryParams params)
            throws A2AClientException {
        JSONRPCRequest request = new JSONRPCRequest(
                generateRequestId(),
                "2.0",
     ...
     * @throws A2AClientException if the request fails
     */
    public JSONRPCResponse cancelTask(final TaskIDParams params)
            throws A2AClientException {
        JSONRPCRequest request = new JSONRPCRequest(
                generateRequestId(),
                "2.0",
    ...
    public CompletableFuture<Void> sendTaskStreaming(
            final TaskSendParams params,
            final StreamingEventListener listener) {
        return CompletableFuture.runAsync(() -> {
            try {
                ...
                if (response.statusCode() != HTTP_OK) {
                    listener.onError(new A2AClientException("HTTP "
                ...
                for (String line : lines) {
                    ...
                    try {
                        ...
                        if (streamingResponse.error() != null) {
                            ...
                            Integer errorCode = error.code() != null
                                    ? error.code().getValue() : null;
    ...
    public AgentCard getAgentCard() throws A2AClientException {
        try {
            ...
            if (response.statusCode() != HTTP_OK) {
                throw new A2AClientException("HTTP " + response.statusCode()
                        + ": " + response.body());
            }
     ...
     * @throws A2AClientException if the request fails
     */
    private JSONRPCResponse doRequest(final JSONRPCRequest request)
            throws A2AClientException {
        try {
            String requestBody = objectMapper.writeValueAsString(request);
            ...
            if (responseNode.has("error")
                    && !responseNode.get("error").isNull()) {
                error = objectMapper.treeToValue(responseNode.get("error"),
                        JSONRPCError.class);
            }
            ...
            if (responseNode.has("result")
                    && !responseNode.get("result").isNull()) {
                result = objectMapper.treeToValue(responseNode.get("result"),
                        Task.class);
            }

            JSONRPCResponse jsonrpcResponse = new JSONRPCResponse(id, jsonrpc,
                    result, error);
