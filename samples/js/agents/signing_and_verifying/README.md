...
2. **Fetching and Verifying the Cards (Client-Side)**
   ...
   * **Verified:** The client instructs the resolver or client to verify the card's signature. The signature metadata is parsed, the publisher's public key is downloaded from the URL (`jku`) listed in the signature, and the cryptographic signature is verified against the canonicalized card payload.
   ...
   ```text
   # 1. Fetching the public card WITHOUT signature verification
   ...
   # 2. Fetching the public card WITH signature verification
   ...
   # 3. Fetching the extended card WITHOUT signature verification
   ...
   # 4. Fetching the extended card WITH signature verification
   Successfully fetched extended agent card with verification:
   {
     "name": "Signed Agent - Extended Card",
     "description": "Extended card containing additional capabilities of the signed agent.",
     ...
