...
async function main(): Promise<void> {
  ...
  const request: SendMessageRequest = {
    ...
    message: {
      messageId: uuidv4(),
      role: Role.ROLE_USER,
      parts: [{ content: { $case: 'text', value: 'hi' }, mediaType: 'text/plain', filename: '', metadata: {} }],
      taskId: '',
      contextId: '',
      extensions: [],
      metadata: {},
    ...
    if (chunk.payload?.$case === 'artifactUpdate') {
      ...
      if (status) {
        ...
        if (status.message) {
          ...
          const timestamp = status.message.metadata?.[TIMESTAMP_FIELD];
  ...
  if (statusMessages.length === 0) {
    throw new Error('Verification failed: Agent did not emit a status message');
  }

  for (const art of artifacts) {
    const ts = art.metadata?.[TIMESTAMP_FIELD];
    if (ts !== expectedIso) {
      throw new Error(`Verification failed: Expected artifact timestamp ${expectedIso}, got ${ts}`);
    }
  }
  for (const msg of statusMessages) {
    const ts = msg.metadata?.[TIMESTAMP_FIELD];
    if (ts !== expectedIso) {
      throw new Error(`Verification failed: Expected message timestamp ${expectedIso}, got ${ts}`);
    }
  }
