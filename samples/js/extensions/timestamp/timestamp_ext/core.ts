...
import { AgentExecutionEvent, RequestContext } from "@a2a-js/sdk/server";
...
export function findExtensionByUri(
    card: AgentCard,
    uri: string
...
export class TimestampExtension {
    ...
    private getArtifactsAndMessagesInTask(t: Task): (Message | Artifact)[] {
        ...
        if (t.history) {
            results.push(
                ...t.history.filter((m) => m.role === Role.ROLE_AGENT)
            );
        }
        if (t.status?.message) {
