...
export type CredoAgentWithOpenId4Vc = CredoAgent<{
...
export function createCredoAgent(
    ...
    const openid4vc = verifier
        ? new OpenId4VcModule({
              app: verifier.app,
              verifier: { baseUrl: `http://localhost:${verifier.port}/oid4vp` },
          })
        : new OpenId4VcModule({});
...
export async function createDidKidVerificationMethod(
    ...
    const verificationMethod =
        didCreateResult.didState.didDocument?.dereferenceKey(kid, [
            "authentication",
        ]);
    ...
    return {
        ...
        kid,
        verificationMethod,
        publicJwk: Kms.PublicJwk.fromPublicJwk(publicJwk),
    };
}
