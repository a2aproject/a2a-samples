...
const HOLDER_SECRET_KEY = "86213c3d7fc8d4d6754c7a0fd969598e"; // gitleaks:allow
...
interface DiscountVoucherClaims extends VoucherClaims {
...
const VOUCHER_DEFINITIONS: VoucherDefinition[] = [
    ...
    },
    {
        ...
        claims: {
            percent_off: 10,
            expires_at: FAR_FUTURE_EXPIRY,
            voucher_id: "meridian-silver-0002",
        },
    },
    {
        partner: "BARGAIN",
        ...
        claims: {
            percent_off: 50,
            expires_at: FAR_FUTURE_EXPIRY,
            voucher_id: "bargain-bronze-0003",
        },
    },
];
 ...
 */
export async function provisionVouchers(
    credoAgent: CredoAgentWithOpenId4Vc
): Promise<ProvisionedVoucher[]> {
    const { kid: holderKid } = await createDidKidVerificationMethod(
        credoAgent.context,
        HOLDER_SECRET_KEY
    );
    ...
    for (const definition of VOUCHER_DEFINITIONS) {
        ...
        const { kid: issuerKid, did: issuerDid } =
            await createDidKidVerificationMethod(
                credoAgent.context,
