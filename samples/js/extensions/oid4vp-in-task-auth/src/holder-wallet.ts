...
function disclosedClaims(match: DcqlValidCredential): Record<string, unknown> {
    return Object.assign(
        {},
        ...(match.claims.valid_claims ?? []).map((claim) => claim.output)
    );
}
...
function readVoucherClaims(
    ...
    if (!(record instanceof SdJwtVcRecord)) {
        throw new Error(
            `Expected an SD-JWT VC record, got ${record.constructor?.name ?? "unknown"}`
        );
    }
    const compact = record.credentialInstances[0]?.compactSdJwtVc ?? "";
    return credoAgent.sdJwtVc.fromCompact(compact).prettyClaims as Record<
        string,
        unknown
    >;
}
 ...
 * auto-picks the first match, so we reuse its result only as a template for the correct
 * `claimFormat` and swap in the chosen record plus claims for selective disclosure.
 */
function buildChosenEntry(
    template: DcqlCredentialEntry,
    chosen: VoucherOption
): DcqlCredentialEntry {
    return {
        ...template,
...
export async function presentVoucher(
    ...
    const resolved =
        await credoAgent.openid4vc.holder.resolveOpenId4VpAuthorizationRequest(
            requestUri
        );
    ...
    const template =
        credoAgent.openid4vc.holder.selectCredentialsForDcqlRequest(
            queryResult
        );
    ...
    if (!(await choice.confirm(chosen.requestedClaims)))
        return { status: "declined" };

    const entry = buildChosenEntry(template[queryId][0], chosen);
    const result =
        await credoAgent.openid4vc.holder.acceptOpenId4VpAuthorizationRequest({
    ...
    return result.ok
        ? { status: "sent" }
        : { status: "failed", detail: JSON.stringify(result).slice(0, 200) };
}
