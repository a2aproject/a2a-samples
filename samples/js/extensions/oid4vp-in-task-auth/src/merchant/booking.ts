...
function keywordWantsDiscount(text: string): boolean {
...
export async function parseRequest(userText: string): Promise<ParsedRequest> {
    try {
        ...
        console.warn(
            "[Merchant] request parsing failed; using keyword intent + defaults:",
            err
        );
        return {
            wantsPartnerDiscount: keywordWantsDiscount(userText),
            nights: 1,
            roomType: "standard",
        };
    }
