...
export const VOUCHER_DCQL_QUERY = {
    ...
    credential_sets: [
        {
            options: [[VOUCHER_DCQL_QUERY_ID]],
            purpose: "Apply partner discount",
        },
    ],
} satisfies DcqlQuery;
...
export function evaluateVoucherClaims(
    ...
    if (!isTrustedIssuer(issuerDid)) {
        return {
            approved: false,
            reason: `issuer ${issuerDid} is not one of our partner brands`,
        };
    }
    if (expiresAt) {
        ...
        if (!Number.isFinite(expiryTime)) {
            return {
                approved: false,
                reason: `the voucher had an unreadable expiry (${expiresAt})`,
            };
        }
        if (expiryTime < now) {
            return {
                approved: false,
                reason: `the voucher expired on ${expiresAt}`,
            };
        }
    }
    if (!Number.isFinite(percentOff) || percentOff <= 0 || percentOff > 100) {
        return {
            approved: false,
            reason: "the voucher did not disclose a valid discount",
        };
    }

    return {
        approved: true,
        discount: { percentOff, brand: findBrandNameForIssuer(issuerDid) },
    };
}
