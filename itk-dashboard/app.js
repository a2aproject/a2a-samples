document.addEventListener('DOMContentLoaded', () => {
    ...
    const BEHAVIORS = [
        { id: 'send_message_flat', name: 'send_message', streaming: false, label: 'Send Message' },
        { id: 'send_message_streaming', name: 'send_message', streaming: true, label: 'Send Message (Streaming)' },
        { id: 'push_notification', name: 'push_notification', streaming: false, label: 'Push Notification' },
        { id: 'resubscribe', name: 'resubscribe', streaming: true, label: 'Resubscribe' }
    ];
    ...
    sdkTabs.querySelectorAll('.tab-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            sdkTabs.querySelectorAll('.tab-btn').forEach(b => b.classList.remove('active'));
    ...
    async function loadDashboardData(url) {
        console.log(`[ITK-DASHBOARD] loadDashboardData called for SDK: ${activeSDK.toUpperCase()}`);
        ...
        try {
            ...
            console.log(`[ITK-DASHBOARD] JSON successfully loaded. Total runs:`, historyData.length);
        } catch (err) {
            console.error(`[ITK-DASHBOARD] All cascading download streams failed.`, err);
            showNoDataState(`Metrics for the ${activeSDK.toUpperCase()} SDK are not available.`);
            return;
        }

        try {
            if (!Array.isArray(historyData) || historyData.length === 0) {
                console.warn(`[ITK-DASHBOARD] Loaded JSON is empty or invalid:`, historyData);
                showNoDataState(`Metrics for the ${activeSDK.toUpperCase()} SDK are not available.`);
                return;
            }
            ...
            console.log(`[ITK-DASHBOARD] Sorted timeline. Newest run:`, historyData[0].timestamp);
            
            activeRunIndex = 0;
            renderDashboard();
            console.log(`[ITK-DASHBOARD] Dashboard populated successfully!`);
        } catch (err) {
            console.error(`[ITK-DASHBOARD] Error building metrics timeline:`, err);
            showNoDataState(`Metrics for the ${activeSDK.toUpperCase()} SDK are not available.`);
        }
    }
    ...
    function renderActiveRun() {
        ...
        if (scenarios.length === 0) {
            topologiesList.innerHTML = '<div class="panel glass topology-card"><p>No test scenarios executed in this run.</p></div>';
            return;
        }
    ...
    function drawTopologyGraph(svgId, sdks, edges, scenarioName) {
        ...
        const isStar = scenarioName.toLowerCase().includes('star') || numNodes > 3;
        const isChain = scenarioName.toLowerCase().includes('chain') || scenarioName.toLowerCase().includes('linear');
        ...
        if (isStar) {
            ...
            for (let i = 1; i < numNodes; i++) {
                const angle = ((i - 1) / (numNodes - 1)) * 2 * Math.PI - Math.PI / 2;
                nodePositions[i] = {
        ...
        edges.forEach(edge => {
            ...
            if (fromNode && toNode) {
                ...
                line.setAttribute('marker-end', `url(#arrow-${svgId.split('-').pop()})`);
        ...
        sdks.forEach((sdk, idx) => {
            ...
            g.setAttribute('class', `svg-node ${pos.isCenter ? 'center-node' : ''}`);
        ...
        PROTOCOLS.forEach(proto => {
            ...
            BEHAVIORS.forEach(behavior => {
                ...
                const matchingScenarios = groupScenarios.filter(scenario => {
                    ...
                    const matchesStreaming = (scenario.streaming === true) === (behavior.streaming === true);
